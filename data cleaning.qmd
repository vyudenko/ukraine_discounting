...
# 15 (always prefer immediate gains or delayed losses).
  ...
  mutate(No_war_experience = case_when(
...
data_full <- data_clean[, c(15, 18:26, 34, 35, 36, 49, 57, 60, 62)] %>% 
  na.omit()
...
ggplot(data_clean, aes(x = score)) +
  geom_bar() +
  facet_wrap(~Spend_3200) +
  labs(title = "How would you spend 3200?") +
  theme_fivethirtyeight()
```

```{r, warning=FALSE, message=FALSE, echo=FALSE}
ggplot(data_clean, aes(x = News_frequence)) +
  geom_bar() +
  labs(title = "Як часто ви переглядаєте новини щодня?") +
  theme_fivethirtyeight()
```
...
ggplot(data_clean, aes(x = war)) +
  geom_bar() +
  facet_wrap(~Fin_expectation) +
  labs(title = "Exposure to violence and expectations") +
  theme_fivethirtyeight()
```
...
```{r, warning=FALSE, message=FALSE, echo=FALSE}
ggplot(data_clean, aes(birthyear, No_war_experience)) +
  geom_jitter() +
...
mod1 <- lm(score ~ ., data = data_full_cat)
summary(mod1)
```

Відфільтрував датасет (no NAs) по зарплаті 20+ та 20-. No significance, окрім Fin_wellbeing_worse (<0.1) - це dummy (1 - фінансове положення з початку війни погіршилось)
...
Відфільтрував датасет (no NAs) по працюючим/непрацюючим. Непрацюючих тільки 16... краще дивіться на працюючих. Є вже вплив Debts (<0.05) <- good
...
Відфільтрував датасет (no NAs) по тим гендеру. Для жінок Debts є сігніфікант на 0.10, що є дуже сумно. Але це вже щось!
...
mod_female <- lm(score ~ ., data_female)
mod_male <- lm(score ~ ., data_male)
stargazer(mod_female, mod_male, 
          header=FALSE, 
...
chart.Correlation(data_full_cat[,c(1:22)], histogram=F)
```
