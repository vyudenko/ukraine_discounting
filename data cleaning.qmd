...
```{r}
# following Kai Ruggery here 
dat_item <- data %>%
  
  mutate(amount_later16 = case_when(
    Q1 == "Отримати 2080 грн за 3 місяці" & Q1_wait_1 == "Отримати 1600 грн прямо зараз" ~ 2080,
    Q1_now_1 == "Отримати 2160 грн за 3 місяці" ~ 2160,
    Q1_now_2 == "Отримати 1600 грн прямо зараз" | Q1_now_2 == "Отримати 2240 грн за 3 місяці" ~ 2240,
    Q1_wait_1 == "Отримати 2000 грн за 3 місяці" & Q1_wait_1 == "Отримати 1600 грн прямо зараз" ~ 2000,
    Q1_wait_2 == "Отримати 1920 грн за 3 місяці" ~ 1920)) %>% 
  
  mutate(amount_later17 = case_when(
    Q1 == "Отримати 2080 грн за 3 місяціs" & Q1_wait_1 == "Отримати 1600 грн прямо зараз" ~ 2160,
    Q1_now_1 == "Отримати 2160 грн за 3 місяці" ~ 2240,
    Q1_now_2 == "Отримати 2240 грн за 3 місяці" | Q1_now_2 == "Отримати 1600 грн прямо зараз" ~ 2320,
    Q1_wait_1 == "Отримати 2000 грн за 3 місяці" & Q1_wait_2 == "Отримати 1600 грн прямо зараз" ~ 2080,
    Q1_wait_2 == "Отримати 1920 грн за 3 місяці" ~ 2000)) %>% 
  
  mutate(amount_later18 = case_when(
    amount_later16 == 1920  ~ 1920,
    amount_later16 == 2000  ~ 2000,
    amount_later16 == 2080  ~ 2080,
    amount_later16 == 2160  ~ 2160,
    amount_later16 == 2240  ~ 2240
  )) %>% 
  ...
  pivot_longer(Q1:Q5_framing, 
               names_to = c("question")) %>%
  
  # what does this code do? idk
  #
  ...
  mutate (amount_later = case_when(
    question == "Q1" | question == "Q2" ~ 550,
    question == "Q1_now_1" | question == "Q2_wait_1" ~ 600,
    question == "Q1_now_2" | question == "Q2_wait_2" ~ 750,
    question == "Q1_wait_1" | question == "Q2_now_1"~ 510,
    question == "Q1_wait_2" | question == "Q2_now_2"~ 505,
  ...
  mutate (question = paste("Q",rep(1:ncol(select(data, Gain550:"Delay_speedup_2")), nrow(dat)), sep ="")) %>%
...
```{r}
ggplot(data_clean, aes(x = score, fill = Gender)) +
  geom_bar() +
  labs(title = "Score_v1 dist") +
  bbc_style() +
  theme(plot.title = element_text(hjust = 0.5, size = 14, face = "bold", family = "mono"),
        axis.title = element_blank(),
        panel.grid.major = element_line(),
...
```{r}
mod1 <- lm(score ~ Risk_preference + data_clean$ + Gender + 
```
